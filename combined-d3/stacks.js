function plot_it()  {
	...
	predictions_data1.forEach((d,i) => {
		d.index = i;
	});
	var shuffled_data = d3.shuffle(predictions_data1);
	var num_predictions = 5000;
	predictions_data1 = [];
	for(var i = 0; i < num_predictions; i++)
		predictions_data1.push(shuffled_data[i]);
	...
	var cats = d3.set(predictions_data1.map(d => d.gt)).values()
	cats.sort()
	var cat_to_ind = {};
	cats.forEach((d,i) => {
		cat_to_ind[d] = i;
	...
	predictions_data1.forEach(datum =>  {
	...
	d3.selectAll('.thecat').on('mouseover', function(cat_d,i)  {
		...
		for(var i = 0; i < cat_d.values.length; i++)  {
			...
			for(var j = 0; j < bin_values.values.length; j++)
				selected_scores.push(scores_data1[bin_values.values[j].index]);
		}
	...
	d3.selectAll('.tp,.fp').on('mouseover', function(score,i)  {
		...
		scores = scores_data1[score.index];
		var line_datum = [];
		cats.forEach((cat,i) => {
	...
	predictions_data2.forEach((d,i) => {
		d.index = i;
	});
	var shuffled_data = d3.shuffle(predictions_data2);
	var num_predictions = 5000;
	predictions_data2 = [];
	for(var i = 0; i < num_predictions; i++)
		predictions_data2.push(shuffled_data[i]);
	...
	var cats = d3.set(predictions_data2.map(d => d.gt)).values()
	cats.sort()
	var cat_to_ind = {};
	cats.forEach((d,i) => {
		cat_to_ind[d] = i;
	...
	// 1) the bin number, given the score
	// 2) sorting the squares, based on false positives and true positives
	predictions_data2.forEach(datum =>  {
		datum.bin = Math.floor(n_bins*(+(datum.score)));
		if(datum.bin == n_bins)
			datum.bin = n_bins-1;
	});
	...
	d3.selectAll('.thecat').on('mouseover', function(cat_d,i)  {
		...
		for(var i = 0; i < cat_d.values.length; i++)  {
			var bin_values = cat_d.values[i];
			for(var j = 0; j < bin_values.values.length; j++)
				selected_scores.push(scores_data2[bin_values.values[j].index]);
		}
	...
	d3.selectAll('.tp,.fp').on('mouseover', function(score,i)  {
		// grab prediction scores, format for line, similar to above...
		scores = scores_data2[score.index];
		var line_datum = [];
		cats.forEach((cat,i) => {
			line_datum.push([cat,scores[cat]]);
		});
